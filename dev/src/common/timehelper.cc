...
void Timer::Start()
{
	...
	m_start = std::chrono::high_resolution_clock::now(); //CORRECNESS ANNOTATIONS
}
...
void Timer::Stop()
{
	...
	m_finish = std::chrono::high_resolution_clock::now(); //CORRECNESS ANNOTATIONS
}

double Timer::getElapsedTime()
{
	auto elapsed_time = std::chrono::duration_cast<std::chrono::nanoseconds>(m_finish - m_start);

	return elapsed_time.count()*0.000000001;
}
...
ScopedTimer::~ScopedTimer()
{
	...
	fprintf(pfile, "%.15lf ", getElapsedTime()); //CORRECTNESS ANNOTATIONS
